...
```
架构文档 → 技术约束提取 → 设计师交接 → 实施计划制定
```
**核心活动**：
- 将架构决策转化为设计约束条件
- 与design subagent进行技术交接
- 提供架构实施指导和技术支持
- 确保设计方案符合架构原则

**产出物**：
- 技术约束文档（给design subagent使用）
- 架构实施指导文档
- 技术风险评估报告

**协作界面**：
```
architect输出 → design输入：
• 系统架构文档 (正式版本)
• ADR技术决策记录集合
• 技术约束和非功能需求清单
• 组件接口规范和数据模型
...
| 技术领域 | 必须使用 | 禁止使用 | 理由 |
|---------|---------|---------|------|
| 后端框架 | FastAPI/Django | Flask | ADR-001: 性能和生态要求 |
| 数据库 | PostgreSQL | MySQL | ADR-002: 事务和扩展性 |
| 前端框架 | React/Vue | Angular | ADR-003: 团队技能匹配 |

### 非功能需求约束
- **性能要求**: API响应时间≤200ms，页面加载≤2s
- **并发要求**: 支持≥1000并发用户
- **可用性要求**: 99.9%可用性，故障恢复≤5分钟
- **安全要求**: OAuth2.0认证，数据传输加密，SQL注入防护

### 部署环境约束
- **容器化**: 必须使用Docker，支持Kubernetes部署
- **环境划分**: dev/test/prod三环境隔离
- **监控要求**: 日志结构化，指标监控，链路追踪
...
### 关键接口规范
- **API风格**: RESTful API，统一响应格式
- **数据格式**: JSON，字段命名camelCase
- **错误处理**: 统一错误码，国际化错误信息
- **版本控制**: URL版本控制，向后兼容
...
### 开发优先级
1. **P0 (核心功能)**: 用户认证、基础CRUD、核心业务流程
2. **P1 (重要功能)**: 数据导入导出、通知系统、权限管理
3. **P2 (增强功能)**: 报表分析、高级搜索、用户偏好

### 技术风险点
| 风险项 | 风险等级 | 影响 | 缓解措施 |
|-------|---------|------|---------|
| 第三方API集成 | 高 | 功能不可用 | 实现降级方案，Mock服务 |
| 性能瓶颈 | 中 | 用户体验差 | 性能测试，缓存优化 |
| 安全漏洞 | 高 | 数据泄露 | 安全审计，代码扫描 |

### 技术债务控制
- **代码质量**: 覆盖率≥80%，静态分析通过
- **文档要求**: API文档自动生成，架构图及时更新
- **重构计划**: 每个迭代预留20%时间处理技术债务

## design subagent 实施指导

### 设计阶段要求
1. **功能设计**: 基于架构约束进行功能拆分和接口设计
2. **性能设计**: 考虑缓存策略、数据库索引、异步处理
3. **安全设计**: 实现认证授权、数据验证、敏感信息处理
4. **容错设计**: 异常处理、重试机制、降级策略

### 开发计划制定
- **任务拆分**: 按模块和优先级拆分，考虑依赖关系
- **时间估算**: 包含开发、测试、部署、文档时间
- **里程碑**: 与架构交付节点对齐，支持增量交付
- **资源规划**: 考虑技能要求，合理分配人员
